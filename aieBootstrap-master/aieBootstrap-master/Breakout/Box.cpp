...
Box::Box(glm::vec2 position, glm::vec2 velocity, float mass, glm::vec2 halfExtents, glm::vec4 colour, float rotation) : Rigidbody(BOX, position, velocity, rotation, mass) { m_colour = colour; m_extents = halfExtents;  m_moment = 1.0f / 12.0f * (mass * halfExtents.x * halfExtents.y); m_angularDrag = 1.0f; m_elasticity = 0.99f; }
