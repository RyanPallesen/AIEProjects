...
Box::Box(glm::vec2 position, glm::vec2 velocity, float mass, glm::vec2 halfExtents, glm::vec4 colour, float rotation, float elasticity, float linearDrag, float angularDrag) : Rigidbody(BOX, position, velocity, rotation, mass, elasticity, linearDrag, angularDrag) { m_colour = colour; m_extents = halfExtents;  m_moment = 1.0f / 12.0f * (mass * halfExtents.x * halfExtents.y); }
